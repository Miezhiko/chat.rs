...
pub struct FreeGptGenerator;

#[async_trait]
impl Generator for FreeGptGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "FreeGpt: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use FreeGpt now!") }
    }
  }
}
...
mod freegpt_tests {
  use super::*;
  #[tokio::test]
  async fn freegpt_test() {
    let gen = FreeGptGenerator;
    let chat_response =
