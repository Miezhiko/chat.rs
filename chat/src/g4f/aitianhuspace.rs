...
pub struct AItianhuSpaceGenerator;

#[async_trait]
impl Generator for AItianhuSpaceGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "AItianhuSpace: Sorry, I can't generate a response right now."
            reslt = False
          else:
            reslt = True
            result = rspns
...
#[cfg(test)]
mod aitianhuspace_tests {
  use super::*;
  #[tokio::test]
  async fn aitianhuspace_test() {
    let gen = AItianhuSpaceGenerator;
    let chat_response =
