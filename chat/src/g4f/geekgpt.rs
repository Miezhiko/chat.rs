...
pub struct GeekGptGenerator;

#[async_trait]
impl Generator for GeekGptGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "GeekGpt: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use GeekGpt now!") }
    }
  }
}
...
mod geekgpt_tests {
  use super::*;
  #[tokio::test]
  async fn geekgpt_test() {
    let gen = GeekGptGenerator;
    let chat_response =
