...
pub struct Gpt6Generator;

#[async_trait]
impl Generator for Gpt6Generator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "Gpt6: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use Gpt6 now!") }
    }
  }
}
