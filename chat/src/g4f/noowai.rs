...
pub struct NoowAiGenerator;

#[async_trait]
impl Generator for NoowAiGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "NoowAi: Sorry, I can't generate a response right now."
            reslt = False
          else:
            reslt = True
            result = rspns
      ...
      }, Err(_) => { bail!("Failed to to use NoowAi now!") }
    }
  }
}
...
mod noowai_tests {
  use super::*;
  #[tokio::test]
  async fn noowai_test() {
    let gen = NoowAiGenerator;
    let chat_response =
