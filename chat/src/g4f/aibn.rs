...
pub struct AibnGenerator;

#[async_trait]
impl Generator for AibnGenerator {
      ...
      c.run(python! {
        ...
        try:
          ...
          rspns = g4f.ChatCompletion.create( model=g4f.models.gpt_4, messages=messages
                                            ...
                                            , provider=g4f.Provider.Aibn )
          if not rspns:
            result = "Aibn: Sorry, I can't generate a response right now."
            reslt = False
          else:
            reslt = True
            result = rspns
